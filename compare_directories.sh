...
checksum_files(){
        ...
        done < <(find "$1" -xdev -type f -xdev -size +${3}M -size -${4}M)
}
...
show_usage(){
        echo -e "\ncompare_directories.sh \"<source path>\" \"<destination path>\" \"<noclean/clean>\" \"<min filesize megabytes>\" \"<max filesize megabytes>\""
        exit 1
}

if [ -z "$1" ] || [ -z "$2" ] || [ -z "$3" ] || [ -z "$4" ] || [ -z "$5" ]; then echo "ERROR: Please specify the correct flags."; show_usage; fi
...
checksum_files "$source_path" "$source_path_checksum_log" "$minsize" "$maxsize"
checksum_files "$destination_path" "$destination_path_checksum_log" "$minsize" "$maxsize"
